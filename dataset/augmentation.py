...
class SimCLRTrainDataTransform(object):
    ...
    def __init__(
        ...
        branch_trans1, branch_trans2 = list(), list()
        if self.gaussian_blur:
            ...
            branch_trans1 += [
                GaussianBlur(kernel_size=kernel_size, p=1.0),
                Solarize(threshold=0.5, p=0.0),
            ]
            branch_trans2 += [
                GaussianBlur(kernel_size=kernel_size, p=0.1),
                Solarize(threshold=0.5, p=0.2),
            ]

        # data_transforms = transforms.Compose(data_transforms)
        data_trans1 = transforms.Compose(data_transforms+branch_trans1)
        data_trans2 = transforms.Compose(data_transforms+branch_trans2)
        ...
        self.train_trans2 = transforms.Compose([
            data_trans2,
            self.final_transform,
        ])
    ...
    def __call__(self, sample):
        ...
        xi = self.train_trans1(sample)
        xj = self.train_trans2(sample)
...
class Solarize(object):
    def __init__(self, threshold=0.5, p=1.0):
